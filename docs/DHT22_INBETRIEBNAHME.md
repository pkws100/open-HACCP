...
**Live-Backend-Test am 24.09.2026:** Die Version `a2896d6` wurde nach Datenbank-Backup auf `haccp.pow24.org` bereitgestellt; Anwendung, Datenbank und Worker waren gesund. Im Browser wurde „ESP32 DHT22 Test“ vorübergehend umbenannt und erfolgreich auf den ursprünglichen Namen zurückgesetzt. Im Einstellungsformular stellte **Eingaben zurücksetzen** einen noch nicht gespeicherten Grenzwert wieder her. Danach wurden für dieses Testgerät Maximum **50 → 49 → 50 °C** und Übertragung **6 → 2 → 6 Stunden** jeweils versioniert gespeichert. Die endgültigen Vorgaben sind wieder 0–50 °C, 5 Minuten Messung und 6 Stunden Übertragung. Konfiguration **v3** steht bis zur nächsten Geräteverbindung noch aus; eine tatsächliche Übernahme an der Firmware wurde hiermit nicht nachgewiesen. Die Audit-Kette blieb nach den Live-Änderungen gültig.

Der Betreiber hat beim zweiten Gerät nur den DHT22 in der Tiefkühltruhe platziert; ESP32 und eine **UGREEN-PB205-USB-Powerbank mit Aufdruck 145 W** stehen außerhalb. Die zuerst genannte Marke Anker wurde anhand der Fotos korrigiert; der Betreiber las zusätzlich die Modellnummer PB205 ab. **Nach Betreiberangabe schaltet die Powerbank die Versorgung zeitweise ab**; eine unabhängige Messung der 5-V-USB-Ausgabe und der Dauerstromversorgung liegt noch nicht vor. Eine erloschene Ladezustandsanzeige allein beweist keine abgeschaltete Ausgabe. Das Dashboard zeigte am **24.09.2026 um 08:55 Uhr** inzwischen **47 echte Messungen, davon 39 nach dem Einbau im Kältebereich**, zuletzt **−15,3 °C**. Damit sind gültige Sensorwerte von der verlängerten Leitung und vom Backend angenommene Übertragungen nach dem Einbau belegt, aber keine ununterbrochene Stromversorgung und kein seriell beobachteter lokaler ACK. Die zuvor allein sichtbare Raumtemperaturmessung von 23,5 °C beschreibt nur einen älteren Zwischenstand. Der jüngste Wert liegt über dem eingestellten Maximum von −18 °C und muss als angezeigte Abweichung geprüft werden; eine Kalibrierung des Sensors ist noch nicht nachgewiesen.

Auf dem zweiten Board läuft weiterhin das **USB-Profil `0.4.1-esp32-dht22`**. Seine Kennzeichnung als Netzbetrieb beschreibt das geflashte Profil und ist bei Versorgung aus der Powerbank keine verlässliche Angabe zur Energiequelle. Die Version `0.4.1` bewahrt den letzten erfolgreichen Upload-Zeitpunkt und sendet nach einem erneuten Einschalten innerhalb des sechs Stunden langen Uploadintervalls nicht zwingend sofort. Im nun implementierten Quellstand `0.4.2` löst ein echter Power-on oder Reset einen frühen authentifizierten Konfigurationsabruf und eine einmalige Messung mit anschließendem Upload beziehungsweise Diagnose-Heartbeat aus; planmäßige Timer-Wakes behalten den eingestellten Takt und ein laufender Wiederholungs-Backoff bleibt wirksam. Diese neue Startlogik ist an der eingebauten Hardware noch **nicht** geflasht oder nachgewiesen. Ohne dauerhaft geeignete 5-V-Versorgung kann auch sie keine lückenlose Überwachung sicherstellen. Mit sechs Stunden Uploadintervall beginnt die Backend-Offline-Erkennung erst nach **zwölf Stunden** ohne erfolgreichen Kontakt; eine kürzere Alarmverzögerung erfordert einen kürzeren Uploadtakt und dessen bestätigte Übernahme am Gerät.

Die Fotos zeigen eine provisorisch mit Klebeband geführte Sensorleitung durch die Türdichtung und eine frei hängende ESP32-Platine. Vor einem unbeaufsichtigten Dauertest die Dichtung bei geschlossener Tür auf vollständigen Schluss, die Leitung auf Quetschung und die Außenverdrahtung auf Zug, Feuchtigkeit und Kurzschlussgefahr prüfen. Das ersetzt noch keinen gemessenen Kältetest des DHT22.

Nach etwa drei Sekunden Tastendruck an der PB205 leuchtete auf einem weiteren Foto die rote Betriebs-LED des ESP32. Dies belegt nur eine Versorgung im Aufnahmezeitpunkt. Ob die 5-V-Ausgabe nach dem automatischen Ende des Kleinstrommodus und bis zum nächsten Upload erhalten bleibt, ist noch ungeprüft.
...
In der Geräteübersicht können Bedienende unter **Namen & Ort** den sichtbaren Gerätenamen, den Messstellennamen und den Ort ändern. Die technische Geräte-UID, der Messstellen-Code und die gespeicherte Messhistorie bleiben dabei erhalten. **Grenzwerte & Takt** speichert Änderungen als neue Konfigurationsversion; vor dem Speichern stellt **Eingaben zurücksetzen** die zuletzt geladenen Werte wieder her, während **Abbrechen** das Formular schließt. Bereits gespeicherte Messwerte und HACCP-Abweichungen werden durch eine Einstellungsänderung nicht gelöscht. Ein angenommener Messwert außerhalb der aktiven Grenzen eröffnet eine Abweichung; erst ein später angenommener Messwert im Bereich beendet die laufende Temperaturabweichung. Bestätigung und Korrekturmaßnahmen sind gesondert im Bereich **Abweichung** dokumentierbar.
...
Der D1 mini speichert maximal **32** offene Datensätze. Das entspricht bei fünf Minuten Messintervall höchstens **2 h 40 min**, beim serverseitig zulässigen Minimum von 30 Sekunden nur **16 min** ohne erfolgreiche Übertragung. Das ESP32-Ziel hält **64** Datensätze vor, entsprechend **5 h 20 min** bei fünf Minuten oder **32 min** bei 30 Sekunden. Der D1 mini versucht bei **28**, der ESP32 bei **60** offenen Datensätzen einen vorgezogenen Upload; beim ESP32 gilt ein noch kleinerer konfigurierter `max_batch_size` als Schwelle. Vier Plätze bleiben zunächst als Reserve. Mit dem Standardtakt von fünf Minuten Messung und sechs Stunden Übertragung kommt es bei erreichbarem WLAN daher ungefähr nach **2 h 20 min am D1 mini** beziehungsweise **5 h am ESP32** zu einem früheren Upload. Das konfigurierte Übertragungsintervall ist in dieser Einstellung kein garantierter Mindestabstand der Kontakte. Bei weiterem Netzausfall fallen nach Erreichen der vollen Kapazität neue Abtastungen aus und erzeugen eine Diagnose, bis bestätigt hochgeladen werden kann. Diese Zahlen beschreiben nur die Speicherkapazität, keine garantierte Offline-Betriebsdauer.
...
Eine echte Powerbank nur über ihren **geregelten 5-V-USB-Ausgang** mit dem USB-C-Anschluss des ESP32 verbinden; keine Rohzellen an den 5V/VIN-Pin legen. Vor dem Umstecken das Mac-USB-Kabel entfernen. Das derzeit auf dem zweiten Board installierte USB-Profil zeigt auch an einer Powerbank zunächst **Netzbetrieb**. Für die Kennzeichnung **Batteriebetrieb · Batteriewert nicht verfügbar** muss das gesonderte `esp32-wroom-dht22-battery-unmonitored`-Profil einmal per Daten-USB geflasht werden. Dabei weder `erase_flash` noch `uploadfs` ausführen, damit Provisionierung und ausstehende Messungen erhalten bleiben. Eine Fernumschaltung oder OTA-Aktualisierung ist noch nicht implementiert. Nach Betreiberangabe endet die Versorgung durch die gezeigte UGREEN-Powerbank bereits mit dem aktuellen USB-Profil; ein Wechsel des Labels durch Flashen löst dieses Versorgungsproblem nicht. Die Firmware geht auch im USB-Profil zwischen Messungen in Light Sleep und schaltet WLAN ab. Ein denkbares Dauerbetriebsprofil mit aktivem WLAN würde deutlich mehr Energie verbrauchen und könnte die Abschaltung **nicht garantiert** verhindern; es ist derzeit weder erstellt noch an dieser Powerbank geprüft. Für Batteriebetrieb ist eine dauerhaft eingeschaltete, geregelte 5-V-Versorgung nötig, deren Verhalten über mehrere Mess- und Sendezyklen geprüft wird. Erst danach das Batterieprofil **ohne Deep Sleep** erproben. [UGREEN beschreibt die Abschaltung bei geringer Last und modellabhängige Kleinstrommodi](https://uk.ugreen.com/blogs/power-bank/power-bank-faqs-complete-troubleshooting-guide). Für [UGREEN PB205](https://manuals.plus/m/e1cfe9a7f49aa9f8e8505e4c66a8344ddef89f38256d467a11eff5d40ee312c0) beschreibt die Herstelleranleitung einen Kleinstrommodus: Taste ungefähr drei Sekunden halten; der Modus endet nach **3,5 Stunden automatisch**. Das schaltet nicht zwingend sofort die USB-Ausgabe ab, ist bei der hier beobachteten Abschaltung aber keine verlässliche Lösung für einen sechs Stunden dauernden Übertragungszyklus. [UGREEN dokumentiert die Aktivierung des Modus auch für die Produktkennung 90597A](https://eu.ugreen.com/de/products/145-w-powerbank-fur-laptops). **145 W bezeichnet eine Höchstleistung, nicht die Eignung für einen dauerhaft kleinen ESP32-Stromverbrauch.**
