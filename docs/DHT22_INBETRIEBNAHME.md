...
Der Betreiber hat beim zweiten Gerät inzwischen nur den DHT22 in der Tiefkühltruhe platziert; ESP32 und eine **UGREEN-PB205-USB-Powerbank mit Aufdruck 145 W** stehen außerhalb. Die zuerst genannte Marke Anker wurde anhand der Fotos korrigiert; der Betreiber las zusätzlich die Modellnummer PB205 ab. **Nach Betreiberangabe schaltet die Powerbank die Versorgung im Betrieb ab**; eine unabhängige Messung der 5-V-USB-Ausgabe liegt noch nicht vor. Eine erloschene Ladezustandsanzeige allein beweist keine abgeschaltete Ausgabe. Die Modellnummer ist auf den Fotos nicht lesbar. Ein Mac-Anschluss oder dauerhaft eingeschaltetes USB-Netzteil ist derzeit nicht verfügbar. Auch nach einer erneuten Dashboard-Aktualisierung zeigte das Backend weiterhin nur die letzte vor dem Einbau angenommene Messung von **23,5 °C / 46,6 % rF um 23:57 Uhr**. Ohne dauerhafte Versorgung sind weder Messung noch Upload möglich; der Tiefkühlschrank ist damit noch nicht aktiv überwacht. Als vorläufige Abhilfe ist ein dauerhaft eingeschaltetes 5-V-USB-Netzteil oder eine dafür nachweislich geeignete USB-Stromquelle nötig. Zusätzlich bewahrt die Firmware ihren letzten erfolgreichen Upload-Zeitpunkt bei einem Stromwechsel auf und sendet bei 6 Stunden Vorgabe nicht automatisch sofort nach dem Einschalten. Ist die Versorgung rechtzeitig wieder stabil und das WLAN erreichbar, wird regulär etwa **05:57 Uhr** gesendet, eventuell früher wegen Pufferdruck; nach einem späteren Einschalten ist der Sendetermin bereits fällig. Ein neuer Kälte-Messwert, DHT-Signalqualität an der verlängerten Leitung und ein ACK nach dem Einbau wurden noch nicht geprüft. Der noch offene Raumtemperatur-Alarm darf nicht als bestätigte Kühlschranktemperatur interpretiert werden. Mit sechs Stunden Uploadintervall beginnt die Backend-Offline-Erkennung erst nach **zwölf Stunden** ohne erfolgreichen Kontakt; eine kürzere Alarmverzögerung erfordert einen kürzeren Uploadtakt und dessen bestätigte Übernahme am Gerät.
