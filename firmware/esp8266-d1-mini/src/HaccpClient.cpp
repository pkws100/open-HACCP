...
)
{
    ...
    if (time(nullptr) < 1704067200) {
        error = "UTC is not synchronized for TLS validation.";
        return -1;
    }
    BearSSL::X509List trustAnchor(OPEN_HACCP_ROOT_CA);
    BearSSL::WiFiClientSecure transport;
    transport.setTrustAnchors(&trustAnchor);
    transport.setX509Time(time(nullptr));
    HTTPClient http;
...
)
{
    ...
    bool seen[DeviceState::BatchCapacity]{};
    bool rejected[DeviceState::BatchCapacity]{};
    for (JsonObjectConst rejection : responseDocument["rejections"].as<JsonArrayConst>()) {
        const size_t index = rejection["index"] | itemCount;
        // Contradictory ACK + rejection for one request index is ambiguous: retain it.
        if (index < itemCount) rejected[index] = true;
    }
    for (JsonObjectConst acknowledgement : responseDocument["acknowledgements"].as<JsonArrayConst>()) {
        ...
        if (index < itemCount && index < DeviceState::BatchCapacity && !seen[index] && !rejected[index]
            && sequence == items[index].sequence && provisioning.measurementPoint == point
...
)
{
    ...
    candidate.maxBatchSize = static_cast<uint16_t>(min(
        serverBatchSize,
        static_cast<uint32_t>(DeviceState::BatchCapacity)
    ));
...
void HaccpClient::addDeviceStatus(JsonDocument &document, const DeviceDiagnostics &diagnostics)
{
    ...
    info["chip_model"] = "ESP8266";
    info["chip_revision"] = 0;
    info["cpu_cores"] = 1;
    info["flash_bytes"] = ESP.getFlashChipRealSize();
    info["psram_bytes"] = 0;
    ...
    status["sleep_fallbacks_since_report"] = 0;
