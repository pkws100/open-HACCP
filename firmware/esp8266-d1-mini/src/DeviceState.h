...
enum class ConfigApplyStatus : uint8_t { Default = 0, Applied = 1, Rejected = 2 };
enum class SleepMode : uint8_t { None = 0, DeepSleep = 1, Awake = 2 };
...
enum DiagnosticFlag : uint32_t {
    ...
    DiagnosticStorageFailed = 1U << 7,
    DiagnosticSensorReadFailed = 1U << 8,
    DiagnosticSensorReadRepeated = 1U << 9,
};
...
struct OperationalState {
    ...
    int64_t plannedWakeAt = 0;
    int64_t timeAnchorAt = 0;
    ...
    uint32_t noClockRetryDelaySeconds = 0;
    uint8_t retryStep = 0;
    uint8_t consecutiveSensorFailures = 0;
    uint16_t anchorCycles = 0;
...
class DeviceState {
public:
    static constexpr size_t QueueCapacity = 32;
    static constexpr size_t BatchCapacity = 8;
    ...
    void recordTimeAnchor(int64_t epoch);
    void recordPlannedWake(int64_t epoch);
    ...
    bool enqueue(int64_t measuredAt, float temperatureC, float humidityRh);
...
private:
    ...
    void loadQueue();
    bool saveQueue(const QueueState &next);
